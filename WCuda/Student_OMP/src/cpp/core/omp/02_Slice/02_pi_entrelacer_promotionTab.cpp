...
double piOMPEntrelacerPromotionTab(int n)
    ...
    const int NB_THREAD = OmpTools::setAndGetNaturalGranularity(); // Logical.
...
#pragma omp parallel
	...
	tabSommeThread[TID] = sommeThread; // Röduction intra-thread additive par promotion de tableau.
	}
    /*
     * Réduction séquentielle du tableau promu: GRATUIT!
     * Car peu de cases.
     */
    double somme = 0;
